...
//     e.stopPropagation(); // mail open hone se rokega
...
//     try {
//       await fetch(
//         `https://fir-df7be-default-rtdb.firebaseio.com/${cleanID}/${type}/${email.id}.json`,
//         {
//           method: "DELETE",
//         }
//       );
...
//       // Redux state update
//       if (type === "inbox") {
//         dispatch(deleteInboxMail(email.id));
//       } else {
//         dispatch(deleteSentMail(email.id));
//       }
//     } catch (error) {
...
import { deleteInboxMail, deleteSentMail } from "../store/emailSlice";
    ...
    >
      ...
      <div className="flex-1 md:flex gap-x-5 md:ml-4 py-1 overflow-hidden min-w-0">
        <div className="flex gap-x-3 min-w-0">
          ...
          <p className="text-black truncate max-w-full min-w-0">
            {type === "inbox" ? email.from : email.to}
          </p>
        </div>

        <p className="text-sm md:text-md text-black md:ml-5 mt-1 md:mt-0">
          {email.subject.length > 40
            ? email.subject.slice(0, 50) + "..."
            : email.subject}
        </p>
